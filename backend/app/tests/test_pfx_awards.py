...
def test_pfx_awards_calculation():
    ...
    qsos = [
        ...
        QSO(
            call="K1HIJ", band="40M", mode="CW", date="20140202", skcc="12000"
        ),  # K1, 12000 points (lower, won't count)
        # International prefixes
        QSO(
            call="VE1KLM", band="20M", mode="CW", date="20140301", skcc="20000"
        ),  # VE1, 20000 points
        QSO(call="G0NOP", band="40M", mode="CW", date="20140302", skcc="25000"),  # G0, 25000 points
        QSO(
            call="JA1QRS", band="15M", mode="CW", date="20140303", skcc="30000"
        ),  # JA1, 30000 points
        # Multiple contacts same prefix, different numbers
        QSO(call="N6TUV", band="20M", mode="CW", date="20140401", skcc="35000"),  # N6, 35000 points
        QSO(
            call="N6WXY", band="40M", mode="CW", date="20140402", skcc="40000"
        ),  # N6, 40000 points (higher)
