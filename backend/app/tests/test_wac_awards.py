...
from services.skcc import QSO, Member, calculate_wac_awards, get_continent_from_call
...
def test_continent_detection():
    ...
    for call, expected_continent, expected_country in test_cases:
        ...
        ), f"Expected {expected_continent} for {call}, got {continent}"
...
def test_wac_basic():
    ...
    for award in awards:
        print(
            f"  {award.name} ({award.award_type}, band={award.band}) - {award.current_continents}/6"
        )
    ...
    assert (
        overall_wac.achieved
    ), f"WAC award should be achieved, got {overall_wac.current_continents}/6 continents"
    assert (
        overall_wac.current_continents == 6
    ), f"Expected 6 continents, got {overall_wac.current_continents}"
    assert set(overall_wac.continents_worked) == {
        "AF",
        "AS",
        "EU",
        "NA",
        "OC",
        "SA",
    }, f"Unexpected continents: {overall_wac.continents_worked}"

    print(
        f"✓ Overall WAC: {overall_wac.current_continents}/6 continents - {'ACHIEVED' if overall_wac.achieved else 'Not achieved'}"
    )
    ...
    assert (
        band_20m.achieved
    ), f"20M WAC should be achieved, got {band_20m.current_continents}/6 continents"

    print(
        f"✓ 20M Band WAC: {band_20m.current_continents}/6 continents - {'ACHIEVED' if band_20m.achieved else 'Not achieved'}"
    )
...
def test_wac_qrp():
    ...
    assert (
        qrp_wac.achieved
    ), f"QRP WAC should be achieved, got {qrp_wac.current_continents}/6 continents"
    assert (
        qrp_wac.current_continents == 6
    ), f"Expected 6 QRP continents, got {qrp_wac.current_continents}"

    print(
        f"✓ QRP WAC: {qrp_wac.current_continents}/6 continents - {'ACHIEVED' if qrp_wac.achieved else 'Not achieved'}"
    )
    ...
    assert (
        qrp_20m.achieved
    ), f"20M QRP WAC should be achieved, got {qrp_20m.current_continents}/6 continents"

    print(
        f"✓ 20M QRP WAC: {qrp_20m.current_continents}/6 continents - {'ACHIEVED' if qrp_20m.achieved else 'Not achieved'}"
    )
...
def test_wac_date_filter():
    ...
    assert overall_wac is not None, "Overall WAC award not found"
    assert (
        overall_wac.current_continents == 1
    ), f"Expected 1 continent (only post-date QSO), got {overall_wac.current_continents}"
    ...
    print(
        f"✓ Date filtering: {overall_wac.current_continents} continents (filtered out pre-Oct 2011 QSOs)"
    )
...
def test_wac_partial_progress():
    ...
    assert (
        overall_wac.current_continents == 3
    ), f"Expected 3 continents, got {overall_wac.current_continents}"
    assert set(overall_wac.continents_worked) == {
        "AS",
        "EU",
        "NA",
    ...
    print(
        f"✓ Partial progress: {overall_wac.current_continents}/6 continents - Not achieved (as expected)"
    )
    print(f"  Continents worked: {', '.join(sorted(overall_wac.continents_worked))}")
    print(
        f"  Still need: {', '.join(sorted({'AF', 'OC', 'SA'} - set(overall_wac.continents_worked)))}"
    )
