...
async def main():
    ...
    ap.add_argument('--mock-members', nargs='*', help='Inline mock members CALL=NUMBER ... for offline use')
    ...
    if args.members_csv:
        ...
        members = await fetch_member_roster(args.roster_url) if args.roster_url else await fetch_member_roster()
    ...
    if args.json:
        ...
        print(f"QSOs: total={output['total_qsos']} CW={output['total_cw_qsos']} matched={output['matched_qsos']} unmatched={len(output['unmatched_calls'])}")
        ...
        if output['endorsements']:
            ...
            for e in output['endorsements']:
                print(f"   * {e['award']} {e['category']} {e['value']}: {e['current']}/{e['required']}")
