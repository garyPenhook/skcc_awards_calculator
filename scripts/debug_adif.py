...
async def debug_adif_parsing():
    ...
    try:
        ...
        for award in awards.awards:
            ...
            print(
                f"  {award.name}: {award.current}/{award.required} ({'ACHIEVED' if award.achieved else 'in progress'}){desc}"
            )
        ...
        for i, qso in enumerate(qsos[:6]):
            if qso.call in member_by_call:
                ...
                current_desc = {
                    ...
                    "S": "Senator",
                    "None": "No award",
                }.get(current_suffix, current_suffix)
        ...
        for qso in qsos[:10]:
            if qso.skcc:
                ...
                if skcc_match:
                    ...
                    print(
                        f"{qso.call} SKCC:{qso.skcc} -> Number {skcc_num} {'FOUND' if number_found else 'NOT FOUND'} in roster"
                    )
