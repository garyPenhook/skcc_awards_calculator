...
def analyze_tribune_progress(adif_files, members):
    ...
    for q in chronological:
        ...
        print(
            f"{date_str} {time_str}  {q.call:<10} {skcc_str} {status_at_qso or '':<10} {member_num:<8} {result}"
        )
    ...
    if len(tribune_qualified) > 0:
        ...
        sorted_qualified = sorted(
            tribune_qualified.items(), key=lambda x: x[1][0].date or "99999999"
        )
